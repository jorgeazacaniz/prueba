...
<body>
   ...
   <div class="col-sm-3">
    <select id="puesto" name="puesto" class="form-control"  required value="" >
         <option value="">Seleccione Puesto</option>
     <?php
   $sql = mysqli_query($con, "SELECT * FROM puesto");
    while($d = mysqli_fetch_assoc($sql)){
    if($d['idpuesto']==$dato['puesto']){   
    echo '<option value="'.$d['idpuesto'].'" selected>'.$d['puesto'].''.$d['idpuesto'].'</option>';
    }else{
    echo '<option value="'.$d['idpuesto'].'">'.$d['puesto'].'</option>';
    }
    }
    ?>
    </select>
